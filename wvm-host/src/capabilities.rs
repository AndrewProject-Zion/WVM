...
pub const CAPABILITIES: &[Capability] = &[
    ...
    Capability {
        ...
        traps: &[
            ...
            "A save FREEZES the guest CPUs for a few seconds while state is written (measured \
             ~6.5s on a 3.4 GiB machine) and no QMP message arrives during that window. Any client \
             read timeout shorter than the pause fails intermittently, which makes it look like a \
             flaky command rather than a fixed cost.",
